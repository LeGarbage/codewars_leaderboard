...
def main():
    def get_leaderboard(user_ranking, errors):
        ...
        if args.display:
            ...
            for group in groups_list:
                table = Table(title=group, show_edge=False, show_lines=True, expand=True)
                ...
                layout["lower"][group].update(table)
    ...
    parser.add_argument("-t", "--time", type=int, default=0, help="Sets the program to run every x minutes")
    ...
    with Live(get_leaderboard(), screen=True, transient=True) as live:      
        while True:
